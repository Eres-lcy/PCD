...
class SimplerAdapter(BaseEnvAdapter):
    """Pi0 adapter for the image/proprio inputs supplied by the PCD runner."""
    def __init__(
        self,
    ...
    def preprocess(
        ...
        raw_proprio = self.preprocess_proprio(proprio)
...
class BridgeSimplerAdapter(SimplerAdapter):
    ...
    def preprocess_proprio(self, proprio: np.array) -> np.array:
    ...
    def postprocess_gripper(self, action: float) -> float:
        """Convert the policy gripper output to the SimplerEnv convention."""
...
class EDRSimplerAdapter(SimplerAdapter):
    def __init__(self, **kwargs):
        ...
        # Constants
        self.sticky_gripper_num_repeat = 15  # repeated for every individual action, not every action chunk. Control freq is 3Hz, so roughly sticky for 5 seconds.
    ...
    def preprocess_proprio(self, proprio: np.array) -> np.array:
        ...
        gripper_width = proprio[
            7
        ]  # from simpler, 0 for close, 1 for open
        # Fractal proprio uses gripper closedness, unlike Bridge's openness.
        gripper_closedness = (
            1 - gripper_width
        )
        raw_proprio = np.concatenate(
