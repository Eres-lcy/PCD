...
from .pcd_fast_flow_matching import PiZeroPCDFast
...
class PiZeroPCDFastPolicyOutput:
    def __init__(self,
                 cfg_dir,
                 checkpoint_path,
    ...
    def step(self, image, instruction, episode, timestep, segmentation_mask, proprio, env, logger, *args, **kwargs):
        inputs = self.preprocess_inputs(image, instruction, proprio)
        raw_actions = self.forward_actions(inputs, timestep, segmentation_mask)
    ...
    def forward_actions(self, inputs, timestep, segmentation_mask):
        with torch.inference_mode():
            if self.use_naive:
                actions = self.model.infer_action_naive(**inputs)
            else:
                if timestep == 0: 
                    self.model.infer_action_batch(**inputs)
                actions, _ = self.model.infer_action(
                    **inputs,
                    timestep=timestep,
