...
def forward_mixture_layers(
    ...
    # [Batch_Size, Seq_Len, Hidden_Size]
    hidden_states_post_attn, attn_weights = forward_mixture_attn(
        mixtures,
    ...
    return hidden_states_final, attn_weights if return_attn_weights else None
...
def forward_mixture_attn(
    ...
    return attn_outputs_final, attn_weights if return_attn_weights else None


# should have named this `MoE`
class PCDFastJointModel(nn.Module):
    ...
    def forward(
        ...
        for layer_idx in range(self.num_hidden_layers):
            ...
            embeds_all, attn_weights = forward_mixture_layers(
                self.mixtures,
                attention_mask,
                position_ids_all,
                embeds_all,
            ...
            if return_all_hidden_states:
                current_layer_states = {k: v for k, v in embeds_all.items() if v is not None}
                output_hidden_states.append(current_layer_states)
        
        if len(output_attn_weights) > 0:
            output_attn_weights = torch.stack(output_attn_weights, dim=0)  # [Num_Layers, Batch_Size, Num_Heads_Q, Full_Seq_Len, Full_Seq_Len]
        ...
        outputs = {"embeds": hidden_states_all}
        if return_attn_weights:
            outputs["attn_weights"] = output_attn_weights
        if return_caches:
            outputs["kv_caches"] = kv_caches

        if return_all_hidden_states and len(output_hidden_states) > 0:
            stacked_hidden_states = {}
            for name in active_mixture_names: 
                ## we can't use batch to do norm here because forward_norm may be adaptive norm, which do not support batch input
                layer_tensors = []
                for states in output_hidden_states:
                    if name in states and states[name] is not None:
                        normed_state = self.mixtures[name].forward_norm(
                            states[name], time_cond
                        )
                        layer_tensors.append(normed_state)
                        
                if len(layer_tensors) > 0:
                    stacked_hidden_states[name] = torch.stack(layer_tensors, dim=0)
            
            outputs["all_hidden_states"] = stacked_hidden_states
