...
_PCD_ROOT = pathlib.Path(__file__).resolve().parent.parent.parent
_CHECKPOINT_ROOT = pathlib.Path(
    os.environ.get('PCD_CHECKPOINT_ROOT', _PCD_ROOT / 'checkpoint')
)
_SAM2_CHECKPOINT = str(_CHECKPOINT_ROOT / 'sam2.1_hiera_large.pt')
...
_GROUNDING_DINO_CHECKPOINT = str(_CHECKPOINT_ROOT / 'grounding-dino-base')
...
def filter_boxes_by_smallest_area(boxes, scores, labels, iou_threshold=0.5):
    ...
    labels = [labels[i] for i in valid_indices.tolist()]
    ...
    for label in unique_labels:
        ...
        idxs = torch.tensor(idxs, device=boxes.device)
        if len(idxs) == 0: continue
            
        current_areas = areas[idxs]
        
        sorted_indices = torch.argsort(current_areas)
        global_indices = idxs[sorted_indices]
        
        while len(global_indices) > 0:
            current = global_indices[0]
            final_keep_indices.append(current.item())
            
            if len(global_indices) == 1: break
            
            rest_indices = global_indices[1:]
            rest_boxes = boxes[rest_indices]
            
            # lt = max(box1_x1, box2_x1), rb = min(box1_x2, box2_x2)
            lt = torch.max(boxes[current, :2], rest_boxes[:, :2])
            rb = torch.min(boxes[current, 2:], rest_boxes[:, 2:])
            
            wh = (rb - lt).clamp(min=0)
            inter_areas = wh[:, 0] * wh[:, 1]
            
            current_area = areas[current]
            overlap_ratios = inter_areas / (current_area + 1e-6)
            
            mask = overlap_ratios < iou_threshold
            
            global_indices = rest_indices[mask]

    final_keep_indices = sorted(final_keep_indices)
    
    return boxes[final_keep_indices], scores[final_keep_indices], [labels[i] for i in final_keep_indices]
...
def expand_boxes(boxes, image_height, image_width, ratio=0.1):
    ...
    expand_w = widths * (ratio / 2)
    expand_h = heights * (ratio / 2)
    
    new_x1 = boxes[:, 0] - expand_w
    new_y1 = boxes[:, 1] - expand_h
    
    new_x2 = boxes[:, 2] + expand_w
    new_y2 = boxes[:, 3] + expand_h
    
    new_x1 = new_x1.clamp(min=0)
    new_y1 = new_y1.clamp(min=0)
    new_x2 = new_x2.clamp(max=image_width)
    new_y2 = new_y2.clamp(max=image_height)
...
def get_center_points_from_boxes(boxes, ratio_x=0.6, ratio_y=0.4):
    ...
    # target_x = x1 + width * ratio_x
    target_x = boxes[:, 0] + widths * ratio_x
    
    # target_y = y1 + height * ratio_y
    target_y = boxes[:, 1] + heights * ratio_y
    
    center_points = torch.stack([target_x, target_y], dim=1)
    
    point_coords = center_points.unsqueeze(1)
    
    point_labels = torch.ones(point_coords.shape[:2], dtype=torch.int, device=boxes.device)
...
class GroundedSAMPredictor:
    ...
    def predict(self, image, prompts, task):
        ...
        target_list = [_NAME_TO_ALIAS_GDINO.get(p, p) for p in prompts]
        gripper_index = target_list.index(
            _NAME_TO_ALIAS_GDINO.get('robot', 'robot')
        )
        if not task.startswith('google_robot'):
            target_list[gripper_index] = 'black ' + target_list[gripper_index]
        ...
        # Grounding DINO outputs are no longer needed while the much larger
        # SAM2 image encoder runs. Drop them early so its allocations can be
        # reused by SAM2.
...
class TrackingPredictorV2:
    ...
    def add_new_mask_without_output(self, frame_idx, obj_id, mask):
        """Add an initial mask without building an unused video-size result."""
    ...
    def predict(self, image, prompts, task, return_masks=True):
        ...
        # The preflight step has encoded the conditioning frame into tracking
        # memory. The cached FP32 image and backbone outputs are not used again
...
def predict_masks_with_predictor(
    ...
    if not return_masks:
        return None
    return predictor.predict(image, prompts)
