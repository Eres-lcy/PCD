"""PCD-Fast target-mask generation."""
...
class SegmentedImageGenerator:
    ...
    def generate(
        ...
        if self.by == "gt":
            mask = self.get_mask_by_gt(obs)
        else:
            mask = self.get_mask_by_predictor(
                obs,
                task,
    ...
    def reset_mask_and_keep_object_names(self, task_description):
        ...
        self.mask_objects += _ROBOT_NAMES if self.by == "gt" else ["robot"]
    ...
    def _set_points_or_boxes(self, obs):
        ...
        masks = []
        for obj_name in self.mask_objects:
            if obj_name == 'robot':
