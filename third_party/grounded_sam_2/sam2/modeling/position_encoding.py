...
class PositionEmbeddingSine(nn.Module):
    ...
    def __init__(
        ...
        if warmup_cache and torch.cuda.is_available():
            # Populate fixed-resolution entries outside torch.compile so the
            # cache never retains an output owned by a CUDA Graph memory pool.
    ...
    @torch.no_grad()
    def _pe(self, batch_size: int, device: torch.device, *cache_key):
        height, width = cache_key
        if cache_key in self.cache:
            return self.cache[cache_key].to(device)[None].repeat(
                batch_size, 1, 1, 1
            )
        y_embed = (
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self._pe(batch_size, x.device, *cache_key)
